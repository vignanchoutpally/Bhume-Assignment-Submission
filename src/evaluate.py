"""Evaluation utilities to score predictions against example truths."""
...
def score(predictions, village: Village) -> Scorecard:
    """Score predictions against village.example_truths."""
    ...
    for pn in truth.index:
        ...
        if pn in pred_u.index:
            ...
            if status == 'corrected':
                ...
                pg = pred_u.loc[pn, 'geometry']
                # Flagged means keep original, so IoU is equal to official IoU
                rec['iou_pred'] = iou_official
                rec['improvement'] = 0.0
                rec['centroid_err_m'] = o.centroid.distance(t.centroid)
            else:
